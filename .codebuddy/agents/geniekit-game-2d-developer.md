...
2. **Create Backend Files** (copy patterns from reference):
   - `backend/src/lib/hunyuan-chat.ts` - copy from `.codebuddy/skills/hunyuan-chat-integrator/lib/hunyuan-chat.ts` (TokenHub/OpenAI-compatible wrapper)
...
- [ ] **Backend SDK**: `openai` is installed in backend (NOT `tencentcloud-sdk-nodejs-hunyuan`).
- [ ] **SDK Wrapper**: `backend/src/lib/hunyuan-chat.ts` is copied from `.codebuddy/skills/hunyuan-chat-integrator/lib/` (TokenHub/OpenAI-compatible), not a hand-written client.
- [ ] **Message Format**: NPC route builds OpenAI `{ role, content }` messages and reads replies from `choices[0].message.content`.
- [ ] **Streaming**: Streaming uses the wrapper's `chatCompletionsStream(onDelta)` text callback and still emits `{ content, npcId }` SSE to the game frontend.
- [ ] **Model**: Uses the wrapper's `DEFAULT_MODEL`;
